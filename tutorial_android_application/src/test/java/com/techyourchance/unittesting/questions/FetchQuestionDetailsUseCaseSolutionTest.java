...
public class FetchQuestionDetailsUseCaseSolutionTest {
    ...
    // second time before timeout after success - listeners notified from cache
    @Test
    ...
    // second time after timeout after success - listeners notified from endpoint
    @Test
    public void fetchQuestionDetailsAndNotify_secondTimeAfterTimeoutAfterSuccess_listenersNotifiedWithDataFromEndpoint() throws Exception {
        success();
        when(mTimeProviderMock.getCurrentTimestamp()).thenReturn(0l);
        SUT.fetchQuestionDetailsAndNotify(QUESTION_ID_1);
